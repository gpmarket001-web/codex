---
name: google-ads-ecommerce
description: Use esta skill principalmente para gerar prompts, direção criativa e briefing visual de imagens premium para anúncios Google Ads de e-commerce feminino 35+/40+.
---

# Skill: Google Ads E-commerce Feminino Premium

## Objetivo

Criar anuncios Google Ads com alta chance de conversao para e-commerce feminino premium, mantendo:

- Clareza
- Desejo
- Seguranca
- Elegancia
- Conforto
- Oferta clara
- Escassez moderada
- Conformidade com politicas de anuncios
- Estetica boutique
- Comunicacao voltada para mulheres adultas, especialmente 35+/40+

---

## Processo obrigatorio

Sempre que criar anuncio, siga esta ordem:

1. Identificar produto
2. Identificar oferta
3. Identificar publico
4. Identificar principal objecao
5. Criar angulos de copy
6. Criar titulos curtos
7. Criar titulos longos
8. Criar descricoes
9. Criar extensoes ou assets extras quando fizer sentido
10. Sugerir direcao criativa da imagem
11. Revisar limites de caracteres
12. Remover promessas arriscadas

---

## Angulos obrigatorios

Sempre gerar pelo menos estes angulos:

### Angulo 1 - Desejo

Foco em elegancia, visual e estilo.

Exemplos:

- Elegancia em Uma So Peca
- Visual Sofisticado Sem Esforco
- O Look Que Ja Nasce Pronto

### Angulo 2 - Conforto

Foco em uso real.

Exemplos:

- Conforto Para o Dia Todo
- Leveza Para Sua Rotina
- Vista-se Bem Sem Abrir Mao do Conforto

### Angulo 3 - Caimento

Foco em modelagem e percepcao premium.

Exemplos:

- Caimento Premium
- Modelagem Wide Leg Elegante
- Movimento Leve e Sofisticado

### Angulo 4 - Seguranca

Foco em confianca.

Exemplos:

- Compra Segura
- Envio Com Rastreio
- Troca Facilitada
- Atendimento Dedicado

### Angulo 5 - Oferta

Foco em acao com clareza, sem pressao exagerada.

Exemplos:

- Condicao Especial
- Oferta Por Tempo Limitado
- Ultimas Pecas Disponiveis

---

## Formula de titulos curtos

Use modelos como:

- [Produto] + [Beneficio]
- [Beneficio] + [Estilo]
- [Oferta] + [Escassez]
- [Seguranca] + [Compra]
- [Produto] + [Publico]

Exemplos:

- Macacao Wide Leg
- Elegancia Sem Esforco
- Caimento Premium
- Ultimas Pecas
- Compre Com Seguranca
- Visual Sofisticado
- Jeans Leve e Elegante
- Look Pronto em Segundos

Titulos curtos devem ter ate 30 caracteres sempre que forem assets de Google Ads.

---

## Formula de titulos longos

Use modelos como:

- [Produto] com [beneficio principal] para [situacao de uso]
- Vista-se com [beneficio emocional] e [beneficio pratico]
- [Oferta] em peca premium com [seguranca]
- [Produto] elegante para [publico/rotina]

Exemplos:

- Macacao jeans wide leg com caimento elegante para mulheres sofisticadas
- Vista-se com conforto, elegancia e praticidade em uma unica peca
- Peca feminina premium com envio seguro e troca facilitada
- Look elegante para passeios, viagens e momentos especiais

Titulos longos devem ter ate 90 caracteres.

---

## Formula de descricoes

Descricoes devem ter ate 90 caracteres.

Use estrutura:

Beneficio + Seguranca + Urgencia leve

Exemplos:

- Caimento elegante, conforto real e visual sofisticado para o dia a dia.
- Peca premium com envio seguro, troca facilitada e atendimento dedicado.
- Oferta por tempo limitado. Garanta sua peca antes do fim do estoque.
- Visual moderno e confortavel para mulheres que valorizam elegancia.
- Compre com seguranca e receba sua peca com rastreio do pedido.

---

## Banco de titulos curtos

Use e adapte:

- Elegancia Sem Esforco
- Caimento Premium
- Visual Sofisticado
- Look Pronto em Segundos
- Conforto Para o Dia
- Peca Versatil Feminina
- Boutique Feminina Online
- Compra Segura
- Envio Com Rastreio
- Troca Facilitada
- Oferta Especial
- Ultimas Pecas
- Modelagem Elegante
- Leveza e Movimento
- Sofisticacao Diaria

---

## Banco de titulos longos

Use e adapte:

- Vista-se com elegancia, conforto e praticidade em uma unica peca
- Peca feminina premium com caimento sofisticado para o dia a dia
- Visual moderno, confortavel e elegante para mulheres adultas
- Compra segura com envio rastreado e suporte dedicado
- Oferta especial em peca versatil para momentos casuais e elegantes

---

## Banco de descricoes

Use e adapte:

- Caimento elegante, conforto real e visual sofisticado para o dia a dia.
- Peca premium com envio seguro, troca facilitada e atendimento dedicado.
- Visual moderno e confortavel para mulheres que valorizam elegancia.
- Garanta sua peca com condicao especial enquanto houver disponibilidade.
- Compre com seguranca e receba sua peca com rastreio do pedido.

---

## Regras de conformidade Google Ads

Nunca usar:

- Promessas corporais exageradas
- Antes e depois corporal
- "Emagreca"
- "Esconda sua barriga"
- "Corpo perfeito"
- "Afina cintura"
- "Resultado garantido"
- "A melhor loja do Brasil"
- Urgencia falsa
- Desconto falso
- Afirmacoes sem prova

Substituir por:

- Caimento elegante
- Visual sofisticado
- Modelagem confortavel
- Valoriza o look
- Compra segura
- Envio com rastreio
- Condicao especial
- Ultimas pecas disponiveis

Nunca inventar prova social, estoque, garantia, desconto, prazo, brinde ou troca. So usar quando for verdade.

---

## Direcao criativa validada

Para prompts de imagem, usar:

- Fundo monocromatico clean
- Modelo adulta, preferencialmente 35+/40+
- Pose natural andando, sentada ou em movimento
- Tecido com movimento suave
- Corpo inteiro quando possivel
- Bastante espaco ao redor
- Iluminacao suave
- Estetica editorial premium
- Cores sofisticadas: marfim, bege, off-white, areia, nude, cinza claro ou terracota suave
- Pouco ou nenhum texto
- Produto em destaque

Prompt-base:

"Imagem editorial premium para campanha de moda feminina, modelo adulta 35+, usando [produto/cor], pose natural caminhando, tecido em movimento suave, fundo monocromatico clean em tom marfim/bege, iluminacao suave de estudio, corpo inteiro com bastante espaco ao redor, estetica sofisticada tipo boutique, visual minimalista, elegante e sem texto."

---

## Criativo com texto

Usar esta estrutura:

Headline:

"Elegancia Sem Esforco"

Subheadline:

"Caimento premium, conforto e sofisticacao em uma unica peca."

Selo discreto:

"Oferta Especial"

Rodape:

"Envio Seguro - Troca Facilitada"

Regras:

- Pouco texto
- Fonte elegante
- Layout limpo
- Produto como protagonista
- CTA discreto
- Nada poluido
- Nada com aparencia de panfleto

---

## Estrutura para Performance Max

Criar assets por intencao:

### Grupo 1 - Desejo

Foco em elegancia, visual premium e estilo.

### Grupo 2 - Produto

Foco no produto, tecido, caimento, cores e modelagem.

### Grupo 3 - Seguranca

Foco em entrega, rastreio, troca e suporte.

### Grupo 4 - Oferta

Foco em condicao especial, disponibilidade e brinde quando existir.

### Grupo 5 - Prova social

Usar somente quando houver prova real.

---

## Estrutura para Demand Gen

Criativos devem parecer editoriais e nativos, com menos texto e mais percepcao visual.

Priorizar:

- Imagem limpa
- Modelo adulta
- Produto valorizado
- Fundo monocromatico
- Texto curto ou nenhum texto
- Sensacao de marca boutique

Evitar:

- Banner agressivo
- Texto grande demais
- Excesso de desconto
- Layout de marketplace

---

## Estrutura para Search Ads

Criar anuncios com:

- Palavra-chave no titulo quando fizer sentido
- Beneficio claro
- Seguranca
- CTA
- Oferta sem exagero

Exemplo de estrutura:

Titulo 1: Macacao Wide Leg Feminino
Titulo 2: Caimento Premium
Titulo 3: Envio Seguro

Descricao:
"Visual elegante, conforto real e compra segura com rastreio do pedido."

---

## Saida padrao

Quando o usuario pedir anuncios Google Ads, entregar neste formato:

### Estrategia

Resumo do angulo principal.

### Titulos curtos

Lista com 10 a 15 opcoes.

### Titulos longos

Lista com 5 opcoes.

### Descricoes

Lista com 5 opcoes ate 90 caracteres.

### Criativos sugeridos

3 a 5 ideias de imagem ou video.

### Melhor combinacao

Indicar a combinacao mais forte para teste inicial.

### Observacoes

Apontar riscos, melhorias ou ajustes de politica quando necessario.

---

## Checklist final

Antes de finalizar, verificar:

- O anuncio e claro?
- O beneficio aparece rapido?
- A oferta e entendivel?
- Existe seguranca de compra?
- A estetica parece premium?
- O texto respeita limites?
- A promessa e segura?
- A linguagem conversa com mulher adulta?
- O anuncio nao parece golpe?
- O CTA e direto?
- O criativo nao esta poluido?

---

## Regra de ouro

O anuncio deve vender primeiro pela percepcao premium, depois pela oferta.

A cliente precisa pensar:

"Essa peca parece elegante, combina comigo, parece confiavel e a condicao esta boa para comprar agora."
